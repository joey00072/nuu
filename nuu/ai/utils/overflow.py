"""
Detects when assistant message content exceeds context window limits.
Compares token estimates against model context_window to trigger compaction.

Owns: overflow detection and message trimming logic.
Delegates to: nuu.ai.types (AssistantMessage), re for token estimation.

Depends on: nuu.ai.types, re
"""
...
from nuu.ai.types import AssistantMessage
...
def is_context_overflow(
    ...
    if message.stop_reason == "error" and message.error_message:
        is_non_overflow = any(
            p.search(message.error_message) for p in NON_OVERFLOW_PATTERNS
        )
        if not is_non_overflow and any(
            p.search(message.error_message) for p in OVERFLOW_PATTERNS
        ):
            return True
    ...
    if (
        ...
        and message.stop_reason == "length"
